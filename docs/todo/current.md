...
- [x] FR-045 色の画面に自前描画のカラーピッカー（色相 × 彩度／明度）を置く — #62
- [x] FR-049 余白を設定にし、窓を「いまの設定で起こりうる最大」に合わせる（保存形式 v8）— #94
- [x] 複数モニタ: 移動で窓が膨らむのを止め、ドラッグを実ピクセルで決め直す — #95 / #96 / #100
      （境界をまたぐ瞬間の跳ねは残っている — #101）
