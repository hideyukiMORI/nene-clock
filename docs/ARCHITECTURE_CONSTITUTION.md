...
合成ルートは**プロセスそのものの責務**も持つ。端末への出力とプロセスの終了コードを
扱ってよいのは `:app` だけである（ADR 0005）。窓が出る前に失敗したとき、
利用者へ届く経路は端末しか無いため。出してよいのは**起動できなかった理由 1 行**だけで、
ログの手段として使わない。

- 機械強制: **active**（static な可変フィールドの禁止＝ArchUnit、`Class#newInstance()` の禁止＝forbidden-apis）
- 機械強制: **active**（`:app` 以外での標準ストリーム利用＝forbidden-apis ＋ ArchUnit、`System.exit`＝forbidden-apis）
