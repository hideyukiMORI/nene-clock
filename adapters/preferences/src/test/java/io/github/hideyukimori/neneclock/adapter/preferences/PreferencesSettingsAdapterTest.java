...
class PreferencesSettingsAdapterTest {
    ...
    @Test
    void ignoresTheTransparencyThatVersionSixStored() {
        // v6 は透明度を持っていた。v7 で捨てたので、読んでも無視する（ADR 0012）。
        writeVersionThree();
        node.putInt("schemaVersion", 6);
        node.putInt("backgroundRed", 1);
        node.putInt("backgroundGreen", 2);
        node.putInt("backgroundBlue", 3);
        node.putInt("fontAlpha", 10);
        node.putInt("backgroundAlpha", 20);
